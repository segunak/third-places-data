...
## Deployment Authentication (Publish Profile)

The GitHub Actions workflow uses a **Publish Profile** for authentication. This is a simple, reliable method that works well with Azure Functions on Linux Consumption plans.

### Required GitHub Repository Secrets

These are configured in GitHub repo Settings → Secrets and variables → Actions → Secrets tab:

| Secret | Description |
|--------|-------------|
| `AZUREAPPSERVICE_PUBLISHPROFILE` | The publish profile XML downloaded from the Azure Function App |
| `AZURE_FUNCTION_KEY` | The function key for authenticating API requests (used in smoke tests) |

### How to Get the Publish Profile

1. Go to the Azure Portal → Function App → Overview
2. Click **Get publish profile** to download the XML file
3. Copy the entire contents of the XML file
4. Add it as a secret named `AZUREAPPSERVICE_PUBLISHPROFILE` in GitHub

### How Deployment Works

The workflow uses `Azure/functions-action@v1` with:

- `scm-do-build-during-deployment: true` - Enables server-side build
- `enable-oryx-build: true` - Uses Azure's Oryx build system to install Python dependencies
- `respect-funcignore: true` - Honors the `.funcignore` file to exclude unnecessary files

With Oryx build enabled, Azure installs dependencies from `requirements.txt` on the server during deployment. This means you don't need to pre-install packages locally in the workflow.

### Why Publish Profile Instead of OIDC?

While OIDC (OpenID Connect) with App Registration is more "modern," it has issues with Linux Consumption plans:

- When using RBAC authentication, the `functions-action` forces `WEBSITE_RUN_FROM_PACKAGE` with a SAS URL
- This can cause the Oryx build to be skipped entirely
- Results in "malformed content" or "sync trigger failed" errors

The publish profile method is simpler and more reliable for this use case.

## Troubleshooting

Tips on troubleshooting weird stuff with the Azure Function.

### Cryptography Module Breaking Deployments (November 2024)

In late November 2024, the `cryptography` module maintainers released changes that broke Azure Functions deployments. Symptoms include:

- Deployment goes green, but functions disappear from the portal
- "Failed to perform sync trigger on function app. Function app may have malformed content."
- 404 errors when calling endpoints after a "successful" deployment

**Fix**: Pin the cryptography version in `requirements.txt`:

```txt
cryptography==43.0.3
```

This issue affects functions using `azure-identity` or any package that depends on `cryptography`. See the [GitHub issue](https://github.com/Azure/azure-sdk-for-python/issues/38725) for details.

### DNS Resolution Errors

If the local deploy from VSCode is showing `getaddrinfo enotfound` as an error message, flush your DNS using `ipconfig /flushdns` and wait a minute or two before trying again.

### Functions Disappear After Deployment

### Functions Disappear After Deployment

I ran into the issue where you deploy the Azure Function, either from VSCode locally or from a GitHub action, and the deployment goes green, but the Azure Function disappears from the portal. What an annoying issue that is. The reasons for this can be numerous:

1. **Cryptography module issue** - See above. Pin `cryptography==43.0.3`.
2. **Wrong folder structure** - Read through [this thread](https://github.com/Azure/azure-functions-python-worker/issues/1338) and [this thread](https://github.com/Azure/azure-functions-python-worker/issues/1262) and [this thread](https://www.reddit.com/r/AZURE/comments/1cuz049/azure_function_swallows_a_build_error_no_way_to/) for things people have done to fix this issue.

In my case, the structure of the directory being uploaded when I started using a GitHub action was wrong. The GitHub action I had initially was generated by Azure using their GitHub integration feature, and it was wrong. Deploying locally from VSCode though I had things working, and so using the "Azure" tab in VSCode (no idea how to do this in the portal) I browsed the function app after a successful local deploy to peep the structure of the folder. Screenshot below. Your end state must be Python packages from your requirements file installed to a `.python_packages` folder in the root, and all other files in the root. I think you could get away with a folder such as `shared_code` to reference modules, but I was having issues with that. This was all with V2 of Python Azure Functions.
