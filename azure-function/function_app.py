...
async def refresh_all_photos(req: func.HttpRequest, client) -> func.HttpResponse:
    """
    HTTP-triggered function that initiates the photo refresh orchestration for all places.
    
    This function is exposed as a public endpoint at /api/refresh-all-photos.
    It starts a new orchestration to refresh photos using cached data files (not live APIs) that:
    ...
    - sequential_mode: If 'true', processes places sequentially rather than in parallel (default: 'false')
...
def refresh_all_photos_orchestrator(context: df.DurableOrchestrationContext):
    """
    Orchestrator function that coordinates refreshing photos for all places.
    
    This orchestrator manages the execution of the photo refresh process with controlled concurrency.
    It retrieves all third places from Airtable and then schedules activity functions
    to refresh photos for each place. It tracks the overall status of the operation.
    
    Args:
        context (df.DurableOrchestrationContext): The durable orchestration context
        
    Returns:
        dict: Results of all photo refresh operations
    """
    try:
        ...
        # Get all places from Airtable using the existing activity function
        all_third_places = yield context.call_activity(
            'get_all_third_places', 
            {"config": config_dict}
        )
        ...
        # Process places based on sequential_mode setting
        if sequential_mode:
...
@app.activity_trigger(input_name="activityInput")
@app.function_name("refresh_single_place_photos")
def refresh_single_place_photos(activityInput):
    """
    Activity function that refreshes photos for a single place.
    
    This function uses a stateless, explicit resource creation system.
    It reads cached data files from GitHub, extracts photos from raw_data,
    applies the photo selection algorithm, and updates both Airtable and GitHub.
    
    Args:
        activityInput: A dictionary containing place information and configuration
    
    Returns:
        dict: The result of the photo refresh operation for this place
    """
    try:
        place = activityInput.get("place")
        ...
        # Initialize components using stateless pattern
        ...
        # Parse raw photos data using same logic as helper function
        photo_list = []
        ...
        # Apply photo selection algorithm
        try:
